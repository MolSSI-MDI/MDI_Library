...
\item Configure QE. You will need to configure QE to be compiled with position-\/independent code. If using G\+CC compilers, you can acomplish this by passing the \char`\"{}\textbackslash{}c -\/f\+P\+I\+C\char`\"{} flag to the \char`\"{}\textbackslash{}c F\+F\+L\+A\+G\+S\char`\"{}, \char`\"{}\textbackslash{}c F\+C\+F\+L\+A\+G\+S\char`\"{}, \char`\"{}\textbackslash{}c C\+F\+L\+A\+G\+S\char`\"{}, and \char`\"{}\textbackslash{}c try\+\_\+foxflags\char`\"{} configuration options. For example\+: \begin{DoxyVerb}cd q-e
...
\begin{DoxyEnumerate}
\item Compile \char`\"{}\textbackslash{}c qemdi.\+x\char`\"{}. \begin{DoxyVerb}make mdi
\end{DoxyVerb}

\item Prepare a QE input file that specifies the initial molecular geometry of the system and any other desired settings.
\item Launch QE using the \char`\"{}\textbackslash{}c qemdi.\+x\char`\"{} executable (which is located in \char`\"{}\textbackslash{}c q-\/e/\+M\+D\+I/src\char`\"{} "{\ttfamily -\/mdi"} runtime option, which is described in \mbox{\hyperlink{library_page_library_launching_sec}{Launching Codes with the M\+DI Library}}. For example\+: \begin{DoxyVerb}qemdi.x -in qe.in -mdi "-role ENGINE -name my_name -method TCP -port 8021 -hostname localhost" &
\end{DoxyVerb}

\item The \char`\"{}\textbackslash{}c q-\/e/\+M\+D\+I/src\char`\"{} directory also contains a QE M\+DI Plugin, called \char`\"{}\textbackslash{}c qemdi\char`\"{}.
