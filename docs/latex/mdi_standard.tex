...
\end{DoxyParagraph}
Not all M\+DI engines are required to support each of these nodes, with the exception of the @\+G\+L\+O\+B\+AL node, which every M\+DI engine must support.
...
The engine proceeds to the next node (see \mbox{\hyperlink{mdi_standard_standard_nodes_sec}{Nodes}}). This command is typically not supported at the {\ttfamily @\+G\+L\+O\+B\+AL}  node.\hypertarget{mdi_standard_send_node}{}\doxysubsection{$<$@}\label{mdi_standard_send_node}
...
If the engine is at the {\ttfamily @\+G\+L\+O\+B\+AL}  node, it calculates and sends its total energy to the driver. If the engine has previously calculated the energy of the system, and no intervening commands from the driver could have changed the energy, the engine is permitted to send the previously calculated energy instead of recalculating it.

If the engine is not at the {\ttfamily @\+G\+L\+O\+B\+AL}  node, it sends its most recently calculated total energy to the driver.
...
If the engine is at the {\ttfamily @\+G\+L\+O\+B\+AL}  node, it calculates and sends its total kinetic energy to the driver. If the engine has previously calculated the energy of the system, and no intervening commands from the driver could have changed the energy, the engine is permitted to send the previously calculated energy instead of recalculating it.

If the engine is not at the {\ttfamily @\+G\+L\+O\+B\+AL}  node, it sends its most recently calculated total kinetic energy to the driver.
...
If the engine is at the {\ttfamily @\+G\+L\+O\+B\+AL}  node, it calculates and sends the total kinetic energy of all electrons in its system to the driver. If the engine has previously calculated the energy of the system, and no intervening commands from the driver could have changed the energy, the engine is permitted to send the previously calculated energy instead of recalculating it.

If the engine is not at the {\ttfamily @\+G\+L\+O\+B\+AL}  node, it sends its most recently calculated electronic kinetic energy to the driver.
...
If the engine is at the {\ttfamily @\+G\+L\+O\+B\+AL}  node, it calculates and sends the total kinetic energy of all nuclei in its system to the driver. If the engine has previously calculated the energy of the system, and no intervening commands from the driver could have changed the energy, the engine is permitted to send the previously calculated energy instead of recalculating it.

If the engine is not at the {\ttfamily @\+G\+L\+O\+B\+AL}  node, it sends its most recently calculated nuclear kinetic energy to the driver.
...
If the engine is at the {\ttfamily @\+G\+L\+O\+B\+AL}  node, it calculates and sends its total potential energy to the driver. If the engine has previously calculated the energy of the system, and no intervening commands from the driver could have changed the energy, the engine is permitted to send the previously calculated energy instead of recalculating it.

If the engine is not at the {\ttfamily @\+G\+L\+O\+B\+AL}  node, it sends its most recently calculated total potential energy to the driver.
